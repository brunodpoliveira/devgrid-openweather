...
def extract_fields_from_response(u_id, response):
    ...
    save_to_json(user_id_request, c_id, temp, humidity)
    return (
        user_id_request,
        c_name,
        c_id,
...
def save_to_json(modded_u_id, c_id, temp, humidity):
    ...
    with open("user_id.json", mode='r', encoding='utf-8') as f2:
        f2_result = json.load(f2)
    with open("user_id.json", mode='w', encoding='utf-8') as f2:
        entry = {'USER_ID': modded_u_id, 'REQUEST_DATETIME': json.dumps(d, default=timeconverter)}
...
def main_loop():
    ...
    if request.method == 'GET':
        ...
        # change html file to actually pass something to info
        info = "info transmitted"
        # info = request.args.get('modded_u_id')
        print('info: ', info)
